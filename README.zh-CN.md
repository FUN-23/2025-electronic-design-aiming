...
[最终版本代码](final/ypy_key_jiguang_timer_new.py)：根据团队确认的最终配置，按键启动后约 **2 秒**触发 GPIO 输出。本文件以另行提供的代码为基础，将定时调用、默认参数及相关注释由 4 秒改为 2 秒，保留 `(320, 208)` 瞄准坐标及调整后的 Y 轴方向。该整理副本已通过静态语法检查，尚未重新进行硬件实测。

`examples/` 中的 12 个脚本继续保留为调试中间版本。最终版本采用固定延时触发，不以瞄准误差作为触发条件。
...
- 支持 `RPi.GPIO` 的树莓派与系统环境；具体树莓派型号未记录。
