...
BUZZER_PIN = 27  # 蜂鸣器引脚（BCM编号，可根据实际修改）
...
if __name__ == '__main__':
    ...
    is_aimed = False
    # 允许的误差范围（根据实际精度需求调整）
    AIM_ERROR_THRESHOLD = 5  # X和Y方向误差均小于此值视为瞄准
        ...
        while found:
            ...
            for c in valid_contours:
                ...
                try:
                    ...
                    # ================ 蜂鸣器控制逻辑 ================
                    # 判断是否瞄准（误差在阈值范围内且未触发过）
                    if (abs(error_x) < AIM_ERROR_THRESHOLD
                            and abs(error_y) < AIM_ERROR_THRESHOLD
                            and not is_aimed):
                        print("瞄准成功！触发蜂鸣器...")
                        trigger_buzzer(duration_ms=100)  # 蜂鸣100ms
                        is_aimed = True  # 标记为已触发，避免重复响
                    # 若误差超出阈值，重置瞄准状态
                    elif abs(error_x) >= AIM_ERROR_THRESHOLD or abs(error_y) >= AIM_ERROR_THRESHOLD:
                        is_aimed = False
                    # ==============================================
