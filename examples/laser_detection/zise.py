...
if __name__ == '__main__':
    ...
    try:
        ...
        # 上电后先寻找矩形，未找到则旋转90度继续读寻
        print("Searching for rectangle...")
        found = False
        rotation_count = 0
        max_rotations = 10  # 最大旋转次数
        while not found and rotation_count < max_rotations:
            ...
            if found:
                ...
                send_motor_command(ser_x, 0x02, 1, 32, 450, 100)
                time.sleep(0.5)
        ...
        while found:
            ...
            # 新增：检测紫色激光点
            laser_center = findPurpleLaser(frame2)
            ...
            for c in valid_contours:
                ...
                try:
                    ...
                    cv2.circle(frame, center_orig, 5, (0, 255, 0), -1)

                    coord_text = f"Rect({center_orig[0]}, {center_orig[1]})"
                    cv2.putText(frame, coord_text, (center_orig[0] + 20, center_orig[1] - 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)

                    # 新增：在图像上绘制紫色激光点
                    if laser_center:
                        cv2.circle(frame, laser_center, 8, (255, 0, 255), -1)
                        laser_text = f"Laser({laser_center[0]}, {laser_center[1]})"
                        cv2.putText(frame, laser_text, (laser_center[0] + 20, laser_center[1] + 20),
                    ...
                    # 新增：输出激光点位置
                    if laser_center:
                        print(f"Purple Laser detected at: ({laser_center[0]}, {laser_center[1]})")
